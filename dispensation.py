...
@ExceptionHandler.handle_exception(logger=LOG)
def main():
    ...
    if scan_type == SCAN_TYPE_SCA:
        ...
        for package in SCA_PACKAGES:
            try:
                ...
                raise IndexError(f"Package {package_name} is not found.")
    ...
    if scan_type == SCAN_TYPE_CSEC:
        ...
        for csec_package in  CSEC_PACKAGES:
            ...
            if update_csec_package:
                LOG.info(f"Successfully snoozed {csec_package} for {CSEC_END_DISPENSATION_DATE}")
